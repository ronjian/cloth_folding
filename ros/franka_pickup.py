...
class Manipulator:
    ...
    def move_to_start(self):
        self.arm_group.set_joint_value_target([p * math.pi / 180.0 for p in [0,-45,0,-135,0,90,45]])
        # 执行运动
        self.arm_group.go(wait=True)
        rospy.loginfo("%s | 运动到起始位置完成！" % self.arm_name)

    def move_gripper(self, position):
        self.gripper_group.set_joint_value_target([position, position])  # Panda夹爪有两个对称关节
    ...
    def move_straight(self, target_pose: Pose):
        ...
        if fraction < 1.0:
            rospy.logerr("%s | 规划失败，仅完成%.1f%%路径" % (self.arm_name, fraction * 100))
            return
        # 执行轨迹
